...
def relationship_status(from_member, to_member, social_graph):
    ...
    from_follow = social_graph.get(from_member,{}).get("following",[])
    to_follow = social_graph.get(to_member,{}).get("following",[])

    if to_member in from_follow and from_member in to_follow:
        return "friends"
    elif to_member in from_follow:
        return "follower"
    elif from_member in to_follow:
        return "followed by"
    else:
...
def eta(first_stop, second_stop, route_map):
    ...
    while current_stop != second_stop:
        for (i,j), k in route_map.items():
            if i == current_stop:
                total += k["travel_time_mins"]
                current_stop = j
                break
